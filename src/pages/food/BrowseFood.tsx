...
import { MapPin, Navigation, Search, Grid, Map as MapIcon, ChevronDown, Package, Scale } from "lucide-react";
...
export default function BrowseFood() {
  ...
  useEffect(() => {
    // Archive expired listings silently (ignore errors for unauthenticated users)
    supabase.rpc("archive_expired_listings").then(() => {});
    
    Promise.all([
      fetchFoodListings("available"),
      fetchFoodListings("expiring_soon"),
      fetchFoodListings("expired"),
    ]).then(([avail, expiringSoon, expired]) => {
      const now = new Date();
      const active = [...avail, ...expiringSoon].filter(
        (l) => !l.expires_at || new Date(l.expires_at) > now
      );
      const newlyExpired = [...avail, ...expiringSoon].filter(
        (l) => l.expires_at && new Date(l.expires_at) <= now
      );
      setListings(active);
      setExpiredListings([...newlyExpired, ...expired]);
  ...
  const renderListingCard = (listing: any, isExpired = false) => (
    <Card key={listing.id} className={`cursor-pointer overflow-hidden transition-shadow hover:shadow-lg ${isExpired ? "opacity-60" : ""}`} onClick={() => navigate(`/food/${listing.id}`)}>
  ...
  return (
    <div className="min-h-screen">
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        {/* Expired / Archived section */}
        {expiredListings.length > 0 && (
          <Collapsible open={showExpired} onOpenChange={setShowExpired} className="mt-8">
            <CollapsibleTrigger asChild>
              <Button variant="ghost" className="w-full justify-between text-muted-foreground">
                <span>Archived / Expired ({expiredListings.length})</span>
                <ChevronDown className={`h-4 w-4 transition-transform ${showExpired ? "rotate-180" : ""}`} />
              </Button>
            </CollapsibleTrigger>
            <CollapsibleContent className="mt-4">
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {expiredListings.map((listing) => renderListingCard(listing, true))}
              </div>
            </CollapsibleContent>
          </Collapsible>
        )}
